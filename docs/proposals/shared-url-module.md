...
`ada-url` should stay private to the URL implementation boundary:

- public headers should not include `ada.h` or expose `ada::url`;
...
- tests should cover behavior through `uvp::url`, not `ada-url` directly.
